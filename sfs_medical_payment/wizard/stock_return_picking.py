...
class stock_return_picking_pharmacy(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if context is None:
            context = {}
        res = super(stock_return_picking_pharmacy, self).default_get(cr, uid, fields, context=context)
        record_id = context and context.get('active_id', False) or False
        pharmacy_obj=self.pool.get('medical.prescription.order').browse(cr,uid,record_id)
        if pharmacy_obj :

#            pick_obj = self.pool.get('stock.picking')
#            pick = pick_obj.browse(cr, uid, record_id, context=context)
            if pharmacy_obj.picking_id:
                pick=pharmacy_obj.picking_id
                ...
                for line in pick.move_lines:
                    ...
                    if return_id in fields:
                        res[return_id] = line.product_qty - line.ret_qty
        return res
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        ...
        if context is None:
            context = {}
        res = super(stock_return_picking_pharmacy, self).view_init(cr, uid, fields_list, context=context)
        ...
        if record_id:
            ...
            if pharmacy_obj.picking_id :
                ...
                return res
            if pharmacy_obj.picking_id and pharmacy_obj.picking_id.state not in ['done','confirmed','assigned']:
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
        ...
        """
        res = super(stock_return_picking_pharmacy, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar,submenu=False)
        record_id = context and context.get('active_id', False)
        active_model = context.get('active_model')
        if  active_model != 'medical.prescription.order':
            return res
...
#                pick = pick_obj.browse(cr, uid, record_id)
                ...
                for m in pharmacy_obj.picking_id.move_lines:
                    return_history[m.id] = 0
                    for rec in m.move_history_ids2:
    ...
    def create_returns(self, cr, uid, ids, context=None):
        ...
        #pick = pick_obj.browse(cr, uid, record_id, context=context)
        pharmacy_obj=self.pool.get('medical.prescription.order').browse(cr,uid,record_id)
        pharmacy_line_obj=self.pool.get('medical.prescription.line')
        ...
        for move in pick.move_lines:
            if not new_picking:
                ...
                new_picking = pick_obj.copy(cr, uid, pick.id, {'name':'%s-return' % pick.name,
                        ...
                        'prescription_id' : pharmacy_obj.id})
            new_location=move.location_dest_id.id
            if move.state=='done':
                new_qty = data['return%s' % move.id]
                returned_qty = move.product_qty
                ############
                prev_ret_qty = move.ret_qty ######previous return qty
                if new_qty > returned_qty - prev_ret_qty:
                    raise osv.except_osv(_('Error !'), _('Please Check the Quantity you have given.'))
                ###########
                ...
                if new_qty:
                    ...
                    ########################assuming products/medicines are not repeated
                    pharmacy_line_ids =[pharmacy.id for pharmacy in pharmacy_obj.prescription_line]
                    pharmacy_line_id = pharmacy_line_obj.search(cr,uid,[('id','in',pharmacy_line_ids),('product_id','=',move.product_id.id)])
                    if pharmacy_line_id:
                        read = pharmacy_line_obj.read(cr,uid,pharmacy_line_id[0],['ret_qty','quantity','product_id'])
                        pharmacy_line_obj.write(cr ,uid,pharmacy_line_id[0],{'ret_qty':int(read['ret_qty'])+new_qty})
                        move_obj.write(cr,uid,move.id,{'ret_qty':move.ret_qty + new_qty})
                    ############
