...
class pos_details(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self,form):
        ...
        from_date = self.convert_from_local_datetime(self.cr, self.uid, form['date_start'], '00:00:00')
        to_date = self.convert_from_local_datetime(self.cr, self.uid, form['date_end'], '23:59:59')
        company_id = user_obj.browse(self.cr, self.uid, self.uid).company_id.id
        domain = [('date_order', '>=', from_date), ('date_order', '<=', to_date),
                  ('state', 'in', ['done', 'paid', 'invoiced']), ('company_id', '=', company_id)]
        if user_ids:
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids):
            for pol in pos.lines:
                result = {
                    ...
                    'total': (pol.price_unit * pol.qty ) - pol.discount, 
            ...
            self.discount += pos.discount_amount
        if data:
            return data
        else:
            return {}
...
report_sxw.report_sxw('report.pos.details.inherit', 'pos.order',
                      'addons/point_of_sale_singer/report/pos_details.rml',
                      parser=pos_details, header='internal')
