...
class account_balance(report_sxw.rml_parse):
    ...
    def _get_analytic_ledger(self, account, ctx={}):
        ...
        if account['type'] in ('other','liquidity','receivable','payable'):
            ...
            journal_ids = ', '.join([str(i) for i in ctx['journal_ids']])
            where = """where aml.period_id in (%s) and aa.id = %s and aml.state <> 'draft' and aml.journal_id in (%s)"""%(periods,account['id'], journal_ids)
    ...
    def lines(self, form, level=0):
        ...
        if form.get('cost_center_ids', []):
            cost_center_ids = form['cost_center_ids']
            journal_ids = journal_pool.search(self.cr, self.uid, [('account_cost_id', 'in', cost_center_ids)], context=self.context)
            self.context.update({'journal_ids': journal_ids})
