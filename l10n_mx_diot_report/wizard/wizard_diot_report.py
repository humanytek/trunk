...
class wizard_account_diot_mx(osv.osv_memory):
    ...
    _columns = {
        'name': fields.char('File Name', readonly=True, size=256),
        ...
        'file': fields.binary('TXT', readonly=True, help='This is File for export to SAT'),
        'file_csv': fields.binary('CSV', readonly=True, help='This is the file to collate'),
    ...
    def create_diot(self, cr, uid, ids, context=None):
        ...
        for line in acc_move_line_obj.browse(cr, uid, move_lines_diot,
            ...
            partner_vat = upper((partner_id.vat or '').replace('-', '')
                                .replace('_', '').replace(' ', ''))
            if not partner_vat \
