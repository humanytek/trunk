...
function openerp_pos_screens(instance, module){ //module is instance.point_of_sale
    ...
    module.ProductScreenWidget = module.ScreenWidget.extend({
        ...
        run_applet: function(total){
            document.JSSerial.clear()
            var serialPortList = document.JSSerial.writeByte(12)
            //var serialPortList = document.JSSerial.writeString('Welcome to Apagen\n')
            var serialPortList = document.JSSerial.writeString('Total\n')
            var serialPortList = document.JSSerial.writeByte(24)
            
            
            var total_string = String(total);
            var total_len = total_string.length;
            if (total_len < 20){
                for (i = 20; i> total_len; i--){
                    var serialPortList = document.JSSerial.writeByte(09);
                }
            }
            
            var serialPortList = document.JSSerial.writeString(total_string)
        },
        start: function(){ //FIXME this should work as renderElement... but then the categories aren't properly set. explore why
            ...
            this.product_list_widget = new module.ProductListWidget(this,{
                click_product_action: function(product){
                    if (document.JSSerial){
                        var order = self.pos.get('selectedOrder')
                        var order_total = order.getTotalTaxIncluded()
                        var product_price = product.get('price')
                        var total = order_total + product_price
                        //self.run_applet(total.toFixed(2))
                    }
