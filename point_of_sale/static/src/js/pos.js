openerp.point_of_sale = function(db) {
    ...
    var Pos = Backbone.Model.extend({
        initialize: function(session, attributes) {
            ...
            $.when(this.get_app_data(),this.fetch('pos.category', ['name', 'parent_id', 'child_id']),
                this.fetch('product.product', ['name', 'list_price', 'pos_categ_id', 'taxes_id', 'product_image_small', 'ean13', 'id','price', 'wholesale_discount_amount'], [['pos_categ_id', '!=', false],['price','=',this.get("pricelist_id")]]),
    ...
    var Orderline = Backbone.Model.extend({
        ...
        getPriceWithoutTaxAndDiscount: function() {
            return this.getAllPricesWithoutDiscount().priceWithoutTax;
        },
        ...
        getAllPricesWithoutDiscount: function() {
            var self = this;
            var base = this.get('quantity') * this.get('price')
        ...
        getAllPrices: function() {
            var self = this;
            var base = (this.get('price') - this.get('discount')) * this.get('quantity')
            var totalTax = base;
            var totalNoTax = base;
        ...
        exportAsJSON: function() {
            return {
                ...
                discount_percent: this.get('discount_percent'),
    ...
    var Order = Backbone.Model.extend({
        ...
        initialize: function(attributes){
            ...
            $('button#wholesale_discount').click(_.bind(this.apply_wholesale_discount, this))
            return this;
        },
        events: {
            'change:validated': 'validatedChanged',
        },
        apply_wholesale_discount: function(){
        	var self = this
        	var order_lines = self.get('orderLines')
        	var show_max_alert = false
        	var show_one_click_alert = false
        	for (_i=0; _i<order_lines.models.length; _i++){
        		current_model = order_lines.models[_i]
        		if (current_model.get('checked') === true && current_model.get('may_key_pressed') === true){
        			show_one_click_alert = true
        		}
        		if (current_model.get('checked') === true && current_model.get('may_key_pressed') === false){
        			wholesale_discount_amount = current_model.get('wholesale_discount_amount')
	        		if (!wholesale_discount_amount){
	        			wholesale_discount_amount = 0.00
	        		}
	        		var price_unit = current_model.get('price')
	        		var orginal_discount = current_model.get('discount')
	        		var net_discount = orginal_discount + wholesale_discount_amount
	        		if (price_unit !== 0.00){
	        			var disc_percent = (net_discount / price_unit) * 100
	        		}
	        		else{
	        			var disc_percent = 100
	        		}
	        		if (disc_percent >= 100){
	        			show_max_alert = true
	        			net_discount = orginal_discount
	        			if (price_unit !== 0.00){
	            			var disc_percent = (net_discount / price_unit) * 100
	            		}
	        			else{
        			...
        			current_model.set({'discount': net_discount, 'discount_percent': disc_percent,'may_key_pressed': true})
        		}
        	}
        	if (show_max_alert === true){
    			alert("Máximo 100% de descuento permitido")
    		}
        	if (show_one_click_alert === true){
        		alert("El descuento de mayoreo ya fue aplicado")
    		}
        },
        ...
        setDiscountPercentage:function(){
        	...
        	order_discount = this.getDiscountAmount()
        	order_line_discount = this.getDiscountTotal()
        	total_discount = order_discount + order_line_discount
        	$('#discountamount').html(parseFloat(total_discount).toFixed(2));
        	this.setNetTotal();
        	this.validate_desc();
        	this.setChangeAmount();
        },
        setDiscountAmount:function(){
        	...
        	order_discount = this.getDiscountAmount()
            order_line_discount = this.getDiscountTotal()
            total_discount = parseFloat(order_discount) + parseFloat(order_line_discount)
        	$('#discountamount').html(total_discount.toFixed(2));
        	this.setNetTotal();
        	this.validate_desc();
        	this.setChangeAmount();
        },
        ...
        setNetTotal: function(){
			...
			$('#total-quantity').html(this.getQuantity());
        ...
        getDiscountTotal: function() {
            total_with_discount = this.getTotalTaxExcluded()
            total_without_discount = this.getTotalTaxExcludedWithoutDiscount()
            discount_amount = total_without_discount - total_with_discount
            return discount_amount
        },
        getTotalTaxExcludedWithoutDiscount: function(){
            return (this.get('orderLines')).reduce((function(sum, orderLine) {
                return sum + orderLine.getPriceWithoutTaxAndDiscount();
            }), 0);
        },
    ...
    var OrderlineWidget = db.web.OldWidget.extend({
        ...
        clickHandler: function() {
            var self = this
            var checked = false
            checkbox_ele = self.$element.find('.pos-line-record-selector')
            if (checkbox_ele[0].checked){
                checked = true
            }
        ...
        render_element: function() {
            ...
            if (this.model.get('checked')){
                this.$element.find('.pos-line-record-selector')[0].checked = true
            }
        },
        refresh: function() {
            var self = this
            ...
            checkbox_ele = self.$element.find('.pos-line-record-selector')
            checkbox_ele[0].checked = checked
            this.updateSummary()
        },
        select: function() {
        ...
        updateSummary: function() {
            ...
            totalTaxExcluded = this.order.getTotalTaxExcluded();
            ...
            $('#total_quantity').html(quantity.toFixed(2)).hide().fadeIn();
        },
    });
    ...
    var OrderWidget = db.web.OldWidget.extend({
        ...
        start: function(){
        	this.record = {};
            var self = this
            $('.pos-all-record-selector').change(function(){
                var toggle = true
                if (!this.checked){
                    toggle = false
                }
                current_order_lines_models = self.currentOrderLines.models
                for (_i=0; _i<current_order_lines_models.length; _i++){
                    current_model = current_order_lines_models[_i]
                    current_model.set({'checked': toggle})
                }
                var check_ele = document.getElementsByClassName("pos-line-record-selector")
                for (var _i = 0; _i < check_ele.length; _i++) {
                    check_ele[_i].checked = toggle;
                }
            })
        },
        ...
        setValue: function(val) {
            var param = {};
            raise_error = true
            checked_order_line_eles = this.$element.find('tr')
            current_order_lines = this.currentOrderLines.models
            var line_length = current_order_lines.length
            var org_discount_percent = 0.00
            for (_i=0; _i<current_order_lines.length; _i++){
                current_model = current_order_lines[_i]
                discount_percent = current_model.get('discount_percent')
                discount_amount = current_model.get('discount')
                temp_val = val
                if (current_model.get('checked')){
                    raise_error = false
                    if (this.numpadState.get('mode') === 'discount_percent'){
//                    	if (this.shop.get('selectedOrder').may_key_pressed){
//                        	this.record = {}
//                        	this.shop.get('selectedOrder').may_key_pressed = false
//                        }
                    	if (val >= 100){
                            alert("Máximo 100% de descuento permitido2")
                            alerted = true
                            val = 0
                            _i = current_order_lines.length
                            this.numpadState.set({'buffer': ''})
                            this.record = {}
                        }
                        if (!this.record[_i]){
                        	old_dis_amount = 0.00
                        	...
                        	old_dis_amount = this.record[_i]
                        }
                    	if (val <= old_dis_amount){
                    		old_dis_amount = 0.00
                    		this.record = {}
                    	}
                        this.record[_i] = val
                        price_unit = current_model.get('price')
                        temp_val = (discount_percent - old_dis_amount) + val
                        if (temp_val >= 100){
                        	alert("Máximo 100% de descuento permitido3")
                        	_i = current_order_lines.length
                        	param['discount'] = discount_amount
                        	temp_val = discount_percent
                        }
                        else{
                        	discount_amount = (temp_val/100) * price_unit
                        	param['discount'] = discount_amount
                        }
                    }
                    param[this.numpadState.get('mode')] = temp_val;
                    current_model.set(param)
                }
                if (line_length !== current_order_lines.length){
                    _i=-1
                    line_length = current_order_lines.length
                }
            }
            if (raise_error){
                alert("No seleccion línea de producto")
                this.numpadState.set({'buffer': ""})
            }
            this.updateSummary()
        },
        ...
        updateSummary: function() {
            ...
            total_qty = currentOrder.getQuantity();
            $('#subtotal').html(totalTaxExcluded.toFixed(2)).hide().fadeIn();
            $('#tax').html(tax.toFixed(2)).hide().fadeIn();
            $('#total').html(total.toFixed(2)).hide().fadeIn();
            $('#total_quantity').html(total_qty.toFixed(2)).hide().fadeIn();
        },
    });
