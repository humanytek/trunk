...
class stock_move(osv.osv):
    ...
    _columns = {
                'prodlot_id': fields.many2one('stock.production.lot', 'Production Lot/Part Number', states={'done': [('readonly', True)]}, help="Production lot is used to put a serial number on the production", select=True),
                }
    _constraints = [
    (_check_prodlot_expiration, _('Cannot move an expired part number to internal or customer location'), ['expired']),
    ]
        ...
        if res.get('warning', False):
            ...
            if obj_prodlot_id.expired:
                res['warning'] = {
                    ...
                    'message': _('This product with the mentioned part number has expired'),
                        }
                return {'warning': res['warning'], 'value': res.get('value') and res['value'].update({'prodlot_id': False}) or {'prodlot_id': False}}
